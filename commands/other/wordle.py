    ...
    async def guess(self, itx: Interaction, _: ui.Button):
        ...
        if len(guess) > 5:
            await itx.followup.send(
                f"<@{self.ctx.author.id}> Your guess must be 5 letters only"
            )
            return

        await itx.followup.send(
            file=self.update_image(word=self.word, guess=guess), view=self
        )
        self.tries += 1
        if self.tries == 5:
            self.stop()
...
async def wordle(ctx: commands.Context):
    ...
    buf.seek(0)
    await ctx.reply(
        file=File(buf, filename="board.png"), view=MyView(ctx=ctx, im=im, word="Crane")
    )
